...
import { map, publishReplay, refCount, distinctUntilChanged, filter, first } from 'rxjs/operators';
...
import { appAutoscalerCredentialSchemaKey } from '../../store/autoscaler.store.module';
...
export class EditAutoscalerCredentialComponent implements OnInit, OnDestroy {
  ...
  createCredential() {
    ...
    const updateAppAutoscalerCredentialService: EntityService = this.entityServiceFactory.create(
      ...
      entityFactory(appAutoscalerCredentialSchemaKey),
  ...
  deleteCredential(): Observable<ActionState> {
    this.store.dispatch(
      new DeleteAppAutoscalerCredentialAction(this.applicationService.appGuid, this.applicationService.cfGuid)
    );
    const actionState = selectUpdateInfo(appAutoscalerCredentialSchemaKey,
      this.applicationService.appGuid,
      UpdateAppAutoscalerCredentialAction.updateKey);
